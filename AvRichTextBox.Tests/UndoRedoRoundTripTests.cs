...
public class UndoRedoRoundTripTests
{
   ...
   public async Task InsertParagraph_then_delete_selection_roundtrips()
   {
      ...
      doc.ExecuteEdit(doc.EditBuilder.BuildReplaceRangeAction(doc.GetTextPosFromGlobalIndex(1), doc.GetTextPosFromGlobalIndex(1), [new EditableRun("\r")]));
   ...
   public async Task Backspace_at_start_of_second_paragraph_deletes_paragraph_break_and_merges()
   {
      ...
      doc.Select(0, 0);
      doc.ExecuteEdit(doc.EditBuilder.BuildReplaceRangeAction(doc.GetTextPosFromGlobalIndex(0), doc.GetTextPosFromGlobalIndex(0), [new EditableRun("a\r"), new EditableRun("b")]));
   ...
   public async Task Enter_with_selection_moves_selected_and_suffix_to_new_paragraph()
   {
      ...
      // select "b"
      doc.Select(1, 1);
      doc.ExecuteEdit(doc.EditBuilder.BuildReplaceRangeAction(doc.GetTextPosFromGlobalIndex(1), doc.GetTextPosFromGlobalIndex(2), [new EditableRun("\r")]));
